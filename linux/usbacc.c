...
#define AD_VID 0x18d1
#define PID 0x4ee7

#define GOOGLE_VID 0x18d1
#define ACCESSORY_PID 0x2d01 /* accessory with adb */
#define ACCESSORY_PID_ALT 0x2d01 /* accessory without adb */
...
void *usbRWHdlr(void * threadarg)
{
	...
	unsigned char inBuff[AOA_BUFF_MAX] = {0};
		...
		unsigned char outBuffRead[AOA_BUFF_MAX] = {0};

	FILE *fp = fopen("./testTxt.txt","r");

	// fgets(outBuffRead, AOA_BUFF_MAX, (FILE*)fp);

	while(fgets(outBuffRead, AOA_BUFF_MAX, fp)!=NULL){
		strcat(outBuff,outBuffRead);
	};
	// fprintf(stdout, "STRBUF IS %s\n",outBuff);
	...
	for (;;) {
		...
		// fprintf(stdout, "Sending ACK: %07d \n", index);
		for(int i = 0; i < 10;i++){
			response =
